        ...
        if self.request.user.is_authenticated:
            if task.user.pk == self.request.user.pk:
                ...
                context['answers'] = Answer.objects.filter(
                    Q(task=task) & (Q(responding_user=self.request.user) | Q(make_public_responding_user=True) & Q(make_public_task_admin=True)))
        else:
            context['answers'] = Answer.objects.filter(task=task, make_public_task_admin=True, make_public_responding_user=True)
